...
module Main refines Main_S {
	...
	{:fuel Proof.strongly_one_step<T, S>, 0, 0}
	Main...
	{
		...;
		...
		while ...
			...
			invariant forall v :: v in V ==> Proof.strongly_one_step(S, QS, v)
		{
			...;
			if ... {
				...;
	...
	lemma safety_props_placeholder...
		...
		ensures forall v :: v in V ==> Proof.strongly_one_step(S, QS, v)
	{
		Proof.agreement_lemma(S, QS);
		Proof.validity_lemma(S, QS);
		forall v | v in V
			ensures Proof.strongly_one_step(S, QS, v)
		{
			Proof.strongly_one_step_lemma(S, QS, v);
		}
	}
}

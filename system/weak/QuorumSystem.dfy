...
module QSystem refines QSystem_A {
	...
	lemma quorum_contains_correct_process(QS : QuorumSystem, q : set<nat>)
		returns (id : nat)
		requires valid_quorum_system(QS)
		requires q in QS.C.Q
		ensures id in q && id in QS.R
	{
		var qR :| qR in QS.C.Q && qR <= QS.R;
		id := two_intersection_nonempty(QS.C.Q, q, qR);
	}

	lemma three_intersection_contains_correct_process(QS : QuorumSystem, q1 : set<nat>, q2 : set<nat>, q3 : set<nat>)
		returns (id : nat)
		requires valid_quorum_system(QS)
		requires q1 in QS.C.Q && q2 in QS.C.Q && q3 in QS.C.Q
		ensures id in q1 * q2 * q3 && id in QS.R
	{
		var qR :| qR in QS.C.Q && qR <= QS.R;
		id := four_intersection_nonempty(QS.C.Q, q1, q2, q3, qR);
	}
	...
	{
		var qR :| qR in QS.C.Q && qR <= QS.R;
		id := five_intersection_nonempty(QS.C.Q, q1, q2, q3, q4, qR);
	}
